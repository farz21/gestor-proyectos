...
async function cargarProyectos() {
  try {
    const [resProyectos, resTareas] = await Promise.all([
      // Promesa para obtener proyectos y tareas al mismo tiempo
    ...
    proyectos.forEach((proyecto) => {
      // Iteramos sobre cada proyecto para renderizarlo
      // COMPARACIÓN SEGURA: Convertimos ambos componentes a String para evitar fallos de tipo
      ...
      const claseActiva =
        String(proyecto.id) === String(window.proyectoActivoId)
          ? "tarjeta-activa"
          : "";
      ...
      // Renderizamos cada proyecto con su información y botones de acción
      html += `
...
window.cargarProyectos = cargarProyectos; // Exportamos la función para que pueda ser utilizada en otros módulos
...
window.crearProyecto = async function (event) {
  if (event) event.preventDefault(); // Evita que se recargue la página al enviar el formulario
  ...
  try {
    await axios.post(`${URL_BASE}/proyectos`, datosProyecto); // Enviamos los datos del nuevo proyecto al servidor
...
window.guardarEdicionProyecto = async function (event) {
  ...
  try {
    await axios.put(`${URL_BASE}/proyectos/${id}`, {
      // Usamos PUT para reemplazar completamente los datos del proyecto
      nombre: nuevoNombre,
  ...
  try {
    ...
    for (const tarea of tareasAsociadas) {
      ...
      for (const comentario of resComentarios.data) {
        // Eliminación en cascada de comentarios asociados a la tarea
